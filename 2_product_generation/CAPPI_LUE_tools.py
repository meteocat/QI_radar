...
def make_CAPPI(ds, CAPPI_H: int):
    ...
    logger.debug(f"Creating CAPPI at altitude {CAPPI_H}m from {len(ds.elev.values)} elevations")
    ...
    try:
        ...
        logger.info(f"CAPPI created successfully.")
    
    except Exception:
        logger.error(f"Error creating CAPPI at altitude {CAPPI_H}m.")
        raise
...
def make_LUE(ds, DEM_resampled):
    ...
    logger.debug("Creating LUE (Lowest Usable Elevation)")
    ...
    try:
        ...
        logger.info("LUE created successfully.")
    except Exception:
        logger.error(f"Error creating LUE.")
        raise
